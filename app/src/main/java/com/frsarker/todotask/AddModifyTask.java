...
public class AddModifyTask extends AppCompatActivity {
    ...
    public void init_modify() {
        ...
        if (task != null) {
            task.moveToFirst();
            etTitle.setText(task.getString(1));
            etContent.setText(task.getString(2));
    ...
    public void saveTask(View v) {
        ...
        if (etContent.getText().toString().trim().length() > 0) {
            ...
            if (isModify) {
                mydb.updateTask(task_id, etTitle.getText().toString(), etContent.getText().toString(), new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()));
                Toast.makeText(getApplicationContext(), "Task Updated.", Toast.LENGTH_SHORT).show();
            } else {
                mydb.insertTask(etTitle.getText().toString(), etContent.getText().toString(), new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()));
